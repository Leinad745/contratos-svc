...
class ContratoControllerTest {
    ...
    private Contrato recurso(Long id) {
        Contrato r = new Contrato();
        r.setId(id);
        r.setNombre("Demo");
        r.setPropiedad("valor");
        r.setArrendatario("valor");
        return r;
    }
    ...
    void obtenerInexistenteDevuelve404() throws Exception {
        ...
        mockMvc.perform(get("/api/contratos/1")).andExpect(status().isNotFound());
    }
    ...
    void actualizarExistenteDevuelve200() throws Exception {
        Contrato actualizado = recurso(1L);
    ...
    void eliminarExistenteDevuelve204() throws Exception {
        ...
        mockMvc.perform(delete("/api/contratos/1")).andExpect(status().isNoContent());
    }

    @Test
    void eliminarInexistenteDevuelve404() throws Exception {
        when(service.delete(1L)).thenReturn(false);
        mockMvc.perform(delete("/api/contratos/1")).andExpect(status().isNotFound());
    }
}
