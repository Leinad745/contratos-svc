package cl.duoc.caso11.contratos.cucumber;
...
public class ContratoSteps {
    ...
    private Long id;
    private ResponseEntity<Contrato> respuesta;
    private ResponseEntity<Void> respuestaVoid;

    private String url() {
        return "http://localhost:" + port + "/api/contratos";
    }
    ...
    public void servicioDisponible(String servicio) {
        ResponseEntity<Contrato[]> listado = rest.getForEntity(url(), Contrato[].class);
    ...
    public void crearRecurso(String recurso, String nombre) {
        ResponseEntity<Contrato> creado = rest.postForEntity(
                url(), body(nombre), Contrato.class);
    ...
    public void consultarCreado(String recurso) {
        respuesta = rest.getForEntity(url() + "/" + id, Contrato.class);
    }
    ...
    public void actualizar(String recurso, String nombre) {
        respuesta = rest.exchange(url() + "/" + id, HttpMethod.PUT, body(nombre), Contrato.class);
    }
    ...
    public void consultarEliminado(String recurso, int codigo) {
        ResponseEntity<Contrato> noExiste = rest.getForEntity(url() + "/" + id, Contrato.class);
