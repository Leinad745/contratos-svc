# Contratos — Estrategia de pruebas

## Pruebas unitarias (cobertura JaCoCo 100% LINE)

- **`ContratoServiceTest`** — prueba de servicio con Mockito: listar, obtener, crear, actualizar (presente y ausente) y eliminar (presente y ausente).
- **`ContratoControllerTest`** — prueba del contrato HTTP con `MockMvc`: 200/404/201/201·400/204 en todas las ramas.
- **`OpenApiConfigTest`** — verifica la metadata OpenAPI.

## BDD con Cucumber (alineado a endpoints)

- **Features**: `src/test/resources/cucumber/contratos.feature`
...
- **Steps**: `cucumber/ContratoSteps` — llama a los endpoints reales vía `TestRestTemplate`
